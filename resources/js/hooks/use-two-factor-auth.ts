...
const TWO_FACTOR_QR_CODE_ENDPOINT = '/user/two-factor-qr-code';
const TWO_FACTOR_SECRET_KEY_ENDPOINT = '/user/two-factor-secret-key';
    ...
    const fetchSetupKey = useCallback(async (): Promise<void> => {
        try {
            const { secretKey: key } = await fetchJson<TwoFactorSecretKey>(
                TWO_FACTOR_SECRET_KEY_ENDPOINT,
            );
    ...
    const fetchRecoveryCodes = useCallback(async (): Promise<void> => {
        try {
            ...
            const codes = await fetchJson<string[]>(
                TWO_FACTOR_RECOVERY_CODES_ENDPOINT,
            );
